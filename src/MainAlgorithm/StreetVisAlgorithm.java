 ...
 * @author usero
 */
public class StreetVisAlgorithm extends SimpleApplication {

    boolean isDebugging = false;

    int imageQuality = 2048;
...
//    boolean isWaitingForIntersection = false;
    ...
    public static void main(String[] args) {
        StreetVisAlgorithm app = new StreetVisAlgorithm();
...
//        }, 500); // Delay to let JME finish creating the window
        ...
        allData = preProcessor.preprocess("assets/Scenes/park_irvington_campbell.json", 10, 10, 6);
//        flyCam.setEnabled(false);
        flyCam.setMoveSpeed(15);
        cam.setLocation(new Vector3f(0.545f, 10.017245f, 15.768f));
        initCrossHairs();
...
//        triangulateGeoJSON("assets/Scenes/debug.geojson");
        isLatLon = true;
        triangulateGeoJSON("assets/Scenes/park_irvington_campbell.geojson");
        addBuildings();
        fixStartEndLatOriented();
        cutLonStartEndSorted(0.34f);
            ...
            if (isInitialized == false) {
                if (isRunningInitialize == false) {
                    ...
                    if (initialDeltas.size() > 0) {
                        if (initialDeltas.get(0).isSetup == false) {
                        ...
                        if (isRunningIntersection == true) {
                            if (delta.texture == null) {
                                ...
                                if (otherBetas.get(gPUBetaCounter).isSetup == false) {
                                    ...
                                    gPUIntersector.changeTextures(otherBetaTexT, deltaT);
                                    debugOutput(otherBetaTexT, "intersectionI1.png");
                                    debugOutput(deltaT, "intersectionI2.png");
                                    ...
                                    debugShadowMaterial2.setTexture("ColorMap", deltaT);
                        ...
                        if (constructingBeta.texture != null) {
                            ...
                            if (isNewGuardNeeded == true) {
                                ...
                                for (int t = 0; t < otherBetas.size(); t++) {
                                    if (minGiuardNumber < otherBetas.get(0).parent.guardNumber) {
                                        minGiuardNumber = otherBetas.get(0).parent.guardNumber;
                                        //CONTINUE LATER
                                    }
                                }
                                constructingBeta.parent.guardNumber = minGiuardNumber + 1;
                                ByteBuffer buf = ByteBuffer.allocateDirect(imageQuality * imageQuality * 4);
                                renderer.readFrameBuffer(delta.myFb, buf);
                                Texture2D deltaT = OffScreenPolyToPixelProcessor.deepCopyTextureFromFrameBuffer(buf, imageQuality, imageQuality);
...
//        offView.setOutputFrameBuffer(offBuffer);
    ...
    public void shootForAllBuildings(StreetSegment segment, ColorRGBA color) {
        ...
        //for (int i = 0; i < myMeshes.size(); i++) {
            ...
            shootForBuilding(buildingsMap, i, segment, color, -0.01f);
            System.out.println("shot meshes: " + ((float) i) / myMeshes.size());
        }
        mss.data.put(segment.myIndex, buildingsMap);
    }

    public void shootForBuilding(HashMap<Integer, ShadowPolygon> buildingsMap, int buildingIDX, StreetSegment segment, ColorRGBA color, float offset) {
        ...
        shootToCorners(shadowPolygon, origin, corners, segment, buildingIDX);
        ...
        shootToCorners(shadowPolygon, origin2, corners, segment, buildingIDX);
    ...
    public void shootToCorners(ShadowPolygon shadowPolygon, Vector3f origin, ArrayList<Vector3f> corners, StreetSegment segment, int buildingIDX) {
