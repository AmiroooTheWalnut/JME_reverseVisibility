...
import static jankovicsandras.imagetracer.ImageTracer.imageToSVG;
...
import static jankovicsandras.imagetracer.ImageTracer.saveString;
...
public class VectorToPolygon {
    ...
    public int numProcessors = Runtime.getRuntime().availableProcessors() / 2;
//    public int numProcessors=2;

    public HashMap<Integer, SimplePolygons> imageToPolygons(int[][] input, AllData allData, int layerIndex, boolean isParallel, boolean isSaveResults) {
        HashMap<Integer, SimplePolygons> imgBoundaries = getBoundariesPoints(input, allData, layerIndex, isParallel, isSaveResults);
        HashMap<Integer, SimplePolygons> output = convertToOriginalScale(imgBoundaries);
        return output;
    }

    public HashMap<Integer, SimplePolygons> convertToOriginalScale(HashMap<Integer, SimplePolygons> input) {

        input.forEach((key, val) -> {
            for (int j = 0; j < val.polygons.size(); j++) {
                for (int k = 0; k < val.polygons.get(j).points.size(); k++) {
                    double x = val.polygons.get(j).points.get(k).xIm;
                    double y = val.polygons.get(j).points.get(k).yIm;
                    val.polygons.get(j).points.get(k).yM = ((x / (imgWidth - 1) * scaleWidth) + scaleOffsetX);
                    val.polygons.get(j).points.get(k).xM = ((y / (imgHeight - 1) * scaleHeight) + scaleOffsetY);
                }
            }
        });
...
//        }
    ...
    public HashMap<Integer, SimplePolygons> getBoundariesPoints(int[][] input, AllData allData, int layerIndex, boolean isParallel, boolean isSaveResults) {
        HashMap<Integer, SimplePolygons> allBoundaries = new HashMap();
        if (isParallel == true) {
            int numIterates = ((LayerDefinition) (allData.all_Layers.get(layerIndex))).categories.length - 1;//FIRST CATEGORY IS FOR UNKNOWN

            ParallelVectorToPolyProcess parallelVectorToPoly[] = new ParallelVectorToPolyProcess[numProcessors];

            for (int i = 0; i < numProcessors - 1; i++) {
                parallelVectorToPoly[i] = new ParallelVectorToPolyProcess(this, allData, input, layerIndex, (int) Math.floor(i * ((numIterates) / numProcessors)), (int) Math.floor((i + 1) * ((numIterates) / numProcessors)), i);
            }
            parallelVectorToPoly[numProcessors - 1] = new ParallelVectorToPolyProcess(this, allData, input, layerIndex, (int) Math.floor((numProcessors - 1) * ((numIterates) / numProcessors)), numIterates, numProcessors - 1);

            for (int i = 0; i < numProcessors; i++) {
                //parallelFor[i].myThread.start();
                parallelVectorToPoly[i].myThread.start();
            }
            for (int i = 0; i < numProcessors; i++) {
                try {
                    parallelVectorToPoly[i].myThread.join();
                    System.out.println("thread " + i + " finished for vet to poly!");
                } catch (InterruptedException ie) {
                    System.out.println(ie.toString());
                }
            }
            for (int i = 0; i < numProcessors; i++) {
                allBoundaries.putAll(parallelVectorToPoly[i].localPolygons);
            }

        } else {
            System.out.println("num VDs: " + ((LayerDefinition) (allData.all_Layers.get(layerIndex))).categories.length);
...
//            saveIndexedImageAsPNG(getVDImage(input, i), allData, layerIndex, "temp");
                ...
                if (isSaveResults == true) {
                    saveIndexedImageAsPNG(getVDImage(input, i), allData, layerIndex, "Region_" + i);
                }
                ...
                if (isSaveResults == true) {
                    ...
                    try {
                        saveString("SVG_Region_" + i + ".svg", imageToSVG(image, options, palette));
                    } catch (Exception ex) {
                ...
                allBoundaries.put(i, boundaries);
            }
        }
...
//        }
        ...
        output = filloutAsoluteTheImage(output, allData, numIndices, 35, isSaveResults);
        if (isSaveResults == true) {
            saveIndexedImageAsPNG(output, allData, numIndices, "finalABSFilled");
        }
        output = filloutRemainingHoles(output, allData, 4);
        if (isSaveResults == true) {
            saveIndexedImageAsPNG(output, allData, numIndices, "finalFilled");
        }
        return output;
    }
    
    public int[][] individualClusterToIndexedImage(AllData allData, ArrayList<Double> lats, ArrayList<Double> lons, ArrayList<Integer> clusters, boolean isSaveResults){
        setScaleFactors(allData);
        double ratio = scaleWidth / scaleHeight;
...
//            if(allData.all_Ways[i].)
//        }
        HashSet<Integer> uniqueClusters = new HashSet<>(clusters);
        for (int i = 0; i < lats.size(); i++) {
            int[] imgP = vectorToImage(lons.get(i), lats.get(i), imgWidth, imgHeight);
            int cellIndex = clusters.get(i);
            if (cellIndex != 0) {
                output[imgP[0]][imgP[1]] = cellIndex;
            }
        }
        int numCategories=uniqueClusters.size();
        output = filloutAsoluteTheImage(output, allData, numCategories, 35, isSaveResults);
        if (isSaveResults == true) {
            saveIndexedImageAsPNG(output, allData, numCategories, "finalABSFilled");
        }
        output = filloutRemainingHoles(output, allData, 4);
        if (isSaveResults == true) {
            saveIndexedImageAsPNG(output, allData, numCategories, "finalFilled");
        }
        return output;
    }

    public long[] cBGlayerToIndexedIDImage(AllData allData, int layerIndex, int[][] input) {
        setScaleFactors(allData);
        double ratio = scaleWidth / scaleHeight;

        imgHeight = (int) (imgWidth * (1 / ratio));
        long[] output = new long[((LayerDefinition)(allData.all_Layers.get(layerIndex))).values.length-1];

        for (int i = 0; i < input.length; i++) {
            for (int j = 0; j < input[0].length; j++) {
                int cellIndex = input[i][j]-1;
                if(cellIndex>0){
                    output[cellIndex]=(long)((LayerDefinition)(allData.all_Layers.get(layerIndex))).values[cellIndex];
                }
            }
        }
        return output;
    ...
    public int[][] filloutRemainingHoles(int[][] input, AllData allData, int numIterations) {
...
//                values[7] = input[i - 1][j - 1];//NW
    ...
    public int[][] filloutAsoluteTheImage(int[][] input, AllData allData, int numIndices, int numIterations, boolean isMakeGIF) {
        ...
        if (isMakeGIF == true) {
            try {
                ...
                BufferedImage first = indexedImageToBufferedImage(input, allData, numIndices);
...
//            saveIndexedImageAsPNG(input, allData, layerIndex, "ABSFill_step" + counter);
            if (writer != null) {
                try {
                    writer.writeToSequence(indexedImageToBufferedImage(input, allData, numIndices));
                } catch (IOException ex) {
                    Logger.getLogger(VectorToPolygon.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
    ...
    public BufferedImage indexedImageToBufferedImage(int[][] input, AllData allData, int numIndices) {
    ...
    public void saveIndexedImageAsPNG(int[][] input, AllData allData, int numIndices, String name) {
        BufferedImage image = indexedImageToBufferedImage(input, allData, numIndices);
    ...
    public void saveIndexedImageAsJPG(int[][] input, AllData allData, int numIndices, String name) {
        BufferedImage image = indexedImageToBufferedImage(input, allData, numIndices);
