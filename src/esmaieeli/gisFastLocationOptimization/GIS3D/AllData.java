...
public class AllData implements Serializable {
    ...
    public void resetParallel(int lavaIndex, int resetLayerIndex) {
        if (resetLayerIndex < 0) {
//            System.out.println("No reset layer defined!");
            for (int i = 0; i < all_Nodes.length; i++) {
                all_Nodes[i].lava_value_indexed[lavaIndex] = 0;
                all_Nodes[i].isBurned = false;
                all_Nodes[i].isChecked[lavaIndex] = false;
                all_Nodes[i].isPassed[lavaIndex] = false;
                all_Nodes[i].f_value[lavaIndex] = 0;
                all_Nodes[i].g_value[lavaIndex] = 0;
                all_Nodes[i].h_value[lavaIndex] = 0;
            }
        } else {
            //DENORMALIZE, BREAKS MODULARITY!!!
            double expectedDenormalized = 3;
            //DENORMALIZE, BREAKS MODULARITY!!!
            for (int i = 0; i < all_Nodes.length; i++) {
                all_Nodes[i].lava_value_indexed[lavaIndex] = ((double) all_Nodes[i].layers.get(resetLayerIndex)) * expectedDenormalized;
                all_Nodes[i].isBurned = true;
                all_Nodes[i].isChecked[lavaIndex] = false;
                all_Nodes[i].isPassed[lavaIndex] = false;
                all_Nodes[i].f_value[lavaIndex] = 0;
                all_Nodes[i].g_value[lavaIndex] = 0;
